    ...
    <section id="projects" ref={sectionRef} className="py-20 bg-white dark:bg-gray-900 transition-colors">
      <div className="container mx-auto px-4">
        ...
        >
          ...
          >
            ...
            >
              {Array.from({ length: totalSlides }).map((_, slideIndex) => (
                <div key={slideIndex} className="w-full flex-shrink-0">
                  <div
                    className={`grid gap-8 ${isMobile ? "grid-cols-1" : "lg:grid-cols-3 md:grid-cols-2 grid-cols-1"}`}
                  >
                    {projects
          ...
          <div className="text-center mt-6">
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {isMobile
                ...
                : `Showing ${currentSlide * projectsPerSlide + 1}-${Math.min((currentSlide + 1) * projectsPerSlide, projects.length)} of ${projects.length} projects`}
            </p>
          </div>
          ...
          <div className="text-center mt-4">
            <p className="text-xs text-gray-400 dark:text-gray-500">
              {isAutoPlaying ? "Auto-playing • Tap to pause" : "Paused • Tap away to resume"}
            </p>
          </div>
        </div>
      </div>
